...
int spdy_frame_parse_header(spdy_frame *frame, spdy_data *data)
{
	...
	* (The mask equals 0x10000000, filtering all but the first bit,
	Control Frame - 1; Data Frame - 0.)
	 */
	frame->type = (data->cursor[0] & 0x80) ?
		SPDY_CONTROL_FRAME : SPDY_DATA_FRAME;
                     ...
                     spdy_data *data)
{
	...
	SPDYDEBUG("Frame parsed successfully ");
	return SPDY_ERROR_NONE;
}
...
void spdy_frame_destroy(spdy_frame *frame)
{
	switch (frame->type) {
		case SPDY_CONTROL_FRAME:
			...
			break;
		case SPDY_DATA_FRAME:
			spdy_data_frame_destroy(&frame->frame.data);
			break;
  }
  /*Header is parsed so should set to 0*/
  frame->_header_parsed = 0;
}
