...
#define PHYS_OUTBUFSIZE 1024 /* size of buffer that avoids malloc */

struct spindly_outdata {
	struct list_node node;
	size_t len; /* number of bytes of data provided */
	unsigned char *buffer; /* if not NULL, an allocated pointer with data
	                         instead of buffer */
...
struct spindly_phys
{
	...
	uint32_t pingid;         /* the next pingid to ask for */
	uint32_t received_streamid;         /* the streamid recieved from peer */
