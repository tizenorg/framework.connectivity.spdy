...
static char spdy3_zlib_dictionary[] = {
...
int spdy_zlib_deflate_init(spdy_zlib_context *ctx)
{
	...
	//ret =deflateInit(&ctx->stream,0);
	ret = deflateInit2(&ctx->stream, 9,
		                             Z_DEFLATED, 11, 1,
		                          Z_DEFAULT_STRATEGY	);
	if (ret != Z_OK) {
		return SPDY_ERROR_ZLIB_INFLATE_INIT_FAILED;
	}
	SPDYDEBUG("Deflate inited");
                     ...
                     unsigned char **dest, size_t *dest_size, uint16_t version)
{
	...
	*dest_size = 0;
	...
	do {
		...
		*data_used += ctx->stream.avail_in;
		...
		do {
			...
			* (See zlib documentation at http://www.zlib.net/zlib_how.html */
			ret = deflate(&ctx->stream, flush);
			/* Should only happen if some other part of the application
			* clobbered the memory of the stream. */
			SPDYDEBUG("zlib return =%d",ret);
			ASSERT_RET_VAL(ret != Z_STREAM_ERROR, SPDY_ERROR_INVALID_DATA);
				...
				case Z_NEED_DICT:
					...
					* is used. (May changes in a future version of SPDY.) */
					if (version == SPINDLY_SPDYVER2) {
						ret = deflateSetDictionary(&ctx->stream,
						                 (const uint8_t*)spdy_zlib_dictionary,
						                 sizeof(spdy_zlib_dictionary));
					} else {
						ret = deflateSetDictionary(&ctx->stream,
						                 (unsigned char *)spdy3_zlib_dictionary,
						                 sizeof(spdy3_zlib_dictionary));
					}
					if (ret != Z_OK) {
		...
		* have been used. */
		ASSERT_RET_VAL(ctx->stream.avail_in == 0, SPDY_ERROR_INVALID_DATA);
                      ...
                      uint16_t version)
{
	...
	*dest_size = 0;
	...
	ASSERT_RET_VAL(ctx != NULL, SPDY_ERROR_INVALID_DATA);
