...
int main(int argc, char *argv[])
{
	...
	spindly_add_header(&test_headers, ":path", "/data/feed/api/user/shakthimshree/albumid/5873636168039844753?thumbsize=288u");
	...
	spindly_add_header(&test_headers, "content-type", "application/x-www-form-urlencoded"),
	spindly_add_header(&test_headers, "accept-encoding", "gzip,deflate");


	/* create a new stream on the physical connection */
	spint = spindly_stream_new(phys_client, 0x1, 0, &stream_client, NULL, NULL, &test_headers);
	...
	while (1) {
		...
		if (len>0) {
			...
			if (SPINDLY_DX_DATA == demux.type && SPINDLY_DATA_FLAGS_FIN == demux.msg.data.flags) {
				fwrite(demux.msg.data.datap, demux.msg.data.len, 1, f);
