...
int main(int argc, char *argv[])
{
	...
	/* Delete a  album with album ID as 5807679875862166833 under user shakthimshree*/
	...
	spindly_add_header(&test_headers, ":method", "DELETE");
	spindly_add_header(&test_headers, ":scheme", "https");
	spindly_add_header(&test_headers, ":path", "/data/entry/api/user/shakthimshree/albumid/5873697261784858258");
	...
	spindly_add_header(&test_headers, "accept-charset", "ISO-8859-1,utf-8;q=0.7,*;q=0.3");
	spindly_add_header(&test_headers, "accept-encoding", "gzip,deflate,sdch");
	spindly_add_header(&test_headers, "if-match", "*");
	spindly_add_header(&test_headers, "accept-language", "en-US,en;q=0.8");

	/* create a new stream on the physical connection */
	spint = spindly_stream_new(phys_client, 0x01, 0, &stream_client, NULL, NULL, &test_headers);
